...
namespace RazzleServer.Server.Common
{
    public sealed class World : AWorld
    {
        ...
        public override void Send(PacketWriter pw, GameClient except = null) => Values
            .SelectMany(x => x.Clients.Values)
            .Where(x => x.Key != except?.Key)
            .ToList()
            .ForEach(x => x.Send(pw));

        public override GameCharacter GetCharacterById(int id) => Values
            .SelectMany(x => x.Clients.Values)
            .Cast<GameClient>()
            .Select(x => x.GameCharacter)
            .FirstOrDefault(x => x.Id == id);

        public override GameCharacter GetCharacterByName(string name) => Values
            .SelectMany(x => x.Clients.Values)
            .Cast<GameClient>()
            .Select(x => x.GameCharacter)
