...
{
    [ProviderAlias("MapleConsole")]
    public class MapleConsoleLoggerProvider : ILoggerProvider, ISupportExternalScope
    {
        private readonly ConcurrentDictionary<string, MapleConsoleLogger> _loggers = new ConcurrentDictionary<string, MapleConsoleLogger>();
        ...
        public MapleConsoleLoggerProvider(Func<string, LogLevel, bool> filter, bool includeScopes)
            : this(filter, includeScopes, false)
        {
        }

        public MapleConsoleLoggerProvider(Func<string, LogLevel, bool> filter, bool includeScopes, bool disableColors)
        {
            if (filter == null)
            {
        ...
        public MapleConsoleLoggerProvider(IOptionsMonitor<ConsoleLoggerOptions> options)
        {
        ...
        public MapleConsoleLoggerProvider(IConsoleLoggerSettings settings)
        {
            if (settings == null)
            {
        ...
        private MapleConsoleLogger CreateLoggerImplementation(string name)
        {
            var includeScopes = _settings?.IncludeScopes ?? _includeScopes;
            var disableColors = _disableColors;

            return new MapleConsoleLogger(name, GetFilter(name, _settings), includeScopes ? _scopeProvider : null, _messageQueue)
            {
