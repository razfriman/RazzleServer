...
namespace RazzleServer.Player.Trade
{
    public class MapleTrade
    {
        ...
        public static MapleTrade CreateTrade(TradeType type, MapleCharacter owner)
        {
            ...
            owner.Client.Send(t.GenerateTradeStart(owner, false));
            return t;
        }
        ...
        public void AcceptTrade(MapleCharacter chr)
        {
            ...
            {
                OwnerAccepted = true;
                Partners[0].Client.Send(GenerateTradeAccepted());
            }
            else if (Partners.Count == 1 && chr == Partners[0])
            {
                PartnerAccepted = true;
                Owner.Client.Send(GenerateTradeAccepted());
            }
            else
            {
        ...
        public void Invite(MapleCharacter c, MapleCharacter inviter)
        {
            ...
            {
                inviter.Client.Send(GenerateCharacterBusyMessage(c.Name));
                return;
            }
            c.Client.Send(GenerateTradeInvite(inviter));
        ...
        public void Close(bool finished, bool success) //may be different for non-trade types like shops
        {
            ...
            {
                Owner.Client.Send(GenerateTradeClose(finished, success));
                ...
                if (!success || !finished)
                {
                    foreach (TradeItem item in items[0])
                    {
                        if (item != null)
                        {
                            Owner.Client.Send(MapleInventory.Packets.AddItem(item.Item, item.Item.InventoryType, item.Item.Position)); //force the "hidden" items to reappear
                        }
                    }
                    Owner.EnableActions();
                }
            ...
            {
                chr.Client.Send(GenerateTradeClose(finished, success));
                chr.Trade = null;
                ...
                if (Type == TradeType.TRADE)
                {
                    if (!success || !finished)
                    {
                        ...
                        {
                            if (item == null) continue;
                            chr.Client.Send(MapleInventory.Packets.AddItem(item.Item, item.Item.InventoryType, item.Item.Position));
                        }
                    }
                }
        ...
        public bool AddItem(MapleItem item, byte tradeSlot, short quantity, MapleCharacter itemOwner)
        {
            ...
            if (Type == TradeType.TRADE)
            {
                ...
                Owner.Client.Send(GenerateTradeItemAdd(item, !isOwner, tradeSlot));
                Partners[0].Client.Send(GenerateTradeItemAdd(item, isOwner, tradeSlot));
                return true;
            }
            return false;
        }
        ...
        public void Chat(MapleCharacter talking, string text)
        {
            ...
            {
                Owner.Client.Send(GenerateChatMessage(talking.Name + " : " + text, num));
            }
            foreach (MapleCharacter chr in Partners)
            {
                if (chr.Trade == this)//be sure they accepted an invite
                {
                    chr.Client.Send(GenerateChatMessage(talking.Name + " : " + text, num));
                }
            }
        }
        ...
        public void AddMesos(MapleCharacter chr, int mesos)
        {
            if (Type == TradeType.TRADE && Partners.Count == 1 && Partners[0].Trade == this)
            {
                if (chr == Owner)
                {
                    ...
                    Owner.Client.Send(UpdateTradeMesos(OwnerMesos, 0));
                    Partners[0].Client.Send(UpdateTradeMesos(OwnerMesos, 1));
                ...
                else if (chr == Partners[0])
                {
                    ...
                    Owner.Client.Send(UpdateTradeMesos(PartnerMesos, 1));
                    Partners[0].Client.Send(UpdateTradeMesos(PartnerMesos, 0));
        ...
        {
            var pw = new PacketWriter(ServerOperationCode.PLAYER_INTERACTION);
            pw.WriteByte(0x16);
            pw.WriteByte(0x02);
            pw.WriteByte(0x0);
