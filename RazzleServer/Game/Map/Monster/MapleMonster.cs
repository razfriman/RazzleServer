...
namespace RazzleServer.Map.Monster
{
    public class MapleMonster
    {
        ...
        public void Damage(MapleCharacter from, int damage, bool showHp = true, bool fromPoison = false)
        {
            lock (MobLock)
            {
                ...
                if (from != null)
                {
                    ...
                    if (showHp)
                        from.Client.Send(UpdateHp(ObjectID, HpPercent));
                }
            ...
            lock (MobLock)
            {
                ...
                if (dropOwner.Job == JobConstants.DARKKNIGHT)
                {
                    ...
                    if (buff != null)
                    {
                        ...
                        if (buff.Stacks <= 0)
                        {
                            dropOwner.Client.Send(Skill.ShowBuffEffect(DarkKnight.FINAL_PACT2, dropOwner.Level, null, false));
                        ...
                        else
                        {
                            ...
                            dropOwner.Client.Send(Buff.UpdateFinalPactKillCount(buff.Stacks, remainingTimeMS));
                        }
                    }
                }
                #endregion
        ...
        public void SetController(MapleCharacter chr, int obbjectId, bool aggro)
        {
            ...
            {
                old.Client.Send(RemoveMobControl(obbjectId));
            }
            Controller.SetTarget(chr);
            ControllerHasAggro = aggro;
            chr.Client.Send(SetMobControl(obbjectId, this));
        }
        ...
        public static PacketWriter SpawnMob(int objectId, MapleMonster mob, bool newSpawn = false) //updated v158
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.SPAWN_MONSTER);
        ...
        {
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.KILL_MONSTER);
            pw.WriteInt(objectId);
        ...
        public static PacketWriter SetMobControl(int objectId, MapleMonster mob, bool newSpawn = false) //updated v142
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.SPAWN_MONSTER_CONTROL);
        ...
        {
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.SHOW_MONSTER_HP);
            pw.WriteInt(objectId);
