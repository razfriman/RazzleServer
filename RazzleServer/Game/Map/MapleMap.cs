    ...
    public class MapleMap
    {
        ...
        public void AddCharacter(MapleCharacter chr)
        {
            ...
            if (chr.Party != null)
            {
                ...
                if (partyMembersOnMap.Any())
                {
                    ...
                    {
                        partyMember.Client.Send(MapleParty.Packets.UpdateParty(chr.Party));
                        if (chr == partyMember) continue;
                        partyMember.Client.Send(hpPacket);
                        chr.Client.Send(MapleParty.Packets.UpdatePartyMemberHp(partyMember));
                    }
                }
            }
        }
        ...
        public void ShowCharacters(MapleCharacter chr)
        {
            lock (Characters)
            {
                ...
                {
                    chr.Client.Send(MapleCharacter.SpawnPlayer(characterKVP.Value));
                    chr.Client.Send(PlayerMovementHandler.CharacterMovePacket(chr.ID, new List<Movement.MapleMovementFragment>()));
                }
            }
        }
        ...
        public void HideCharacter(MapleCharacter chr)
        {
            lock (Characters)
            {
                ...
                {
                    characterKvp.Value.Client.Send(MapleCharacter.RemovePlayerFromMap(chr.ID));
                }
            }
        ...
        public void UnhideCharacter(MapleCharacter chr)
        {
            lock (Characters)
            {
                foreach (var characterKVP in Characters.Where(x => x.Key != chr.ID && !x.Value.IsAdmin))
                {
                    characterKVP.Value.Client.Send(MapleCharacter.SpawnPlayer(chr));
                    chr.Client.Send(PlayerMovementHandler.CharacterMovePacket(chr.ID, new List<Movement.MapleMovementFragment>()));
                }
            }
        ...
        public void ShowMobs(MapleCharacter chr)
        {
            ...
            lock (Mobs)
            {
                ...
                {
                    chr.Client.Send(MapleMonster.SpawnMob(kvp.Key, kvp.Value));
                    ...
                    {
                        chr.Client.Send(MapleMonster.RemoveMobControl(kvp.Key));
                    }
                }
            }
        ...
        public void ShowSummons(MapleCharacter chr)
        {
            lock (Summons)
            {
                foreach (MapleSummon summon in Summons.Values)
                    chr.Client.Send(summon.GetSpawnPacket(false));
            }
        }
            ...
            lock (StaticObjects)
            {
                foreach (StaticMapObject obj in StaticObjects.Values)
                {
                    ...
                    if (obj.IsPartyObject)
                    {
                        ...
                        {
                            chr.Client.Send(obj.GetSpawnPacket(false));
                        }
                    }
                    else
                        chr.Client.Send(obj.GetSpawnPacket(false));
        ...
        public void ShowMapItems(MapleCharacter chr)
        {
            lock (MapItems)
            {
                ...
                {
                    chr.Client.Send(MapleMapItem.Packets.SpawnMapItem(kvp.Value, kvp.Value.Position, 2));
                }
            }
        }
                ...
                if (mapItem.PlayerDrop || mapItem.DropType == MapleDropType.FreeForAll || mapItem.DropType == MapleDropType.Boss || mapItem.DropType == MapleDropType.Unk || (mapItem.DropType == 0 && mapItem.OwnerID == c.Account.Character.ID))  //TODO: party, dropType == 1 and ownerId is in player's party
                {
                    ...
                    if (remove)
                    {
                        ...
                        if (mapItem.Item != null)
                            c.Send(MapleInventory.Packets.ShowItemGain(mapItem.Item.ItemId, mapItem.Item.Quantity, true));
                        return true;
                    }
                }
            }
        ...
        public void ShowReactors(MapleCharacter chr)
        {
            foreach (KeyValuePair<int, WzMap.Reactor> ReactorItem in Reactors)
            {
                if (!DespawnedReactors.ContainsKey(ReactorItem.Key))
                    chr.Client.Send(SpawnReactor(ReactorItem.Key, ReactorItem.Value));
            }
        }
        ...
        public void ShowNpcs(MapleCharacter Character)
        {
            ...
            {
                Character.Client.Send(ShowNpc(Npc.Key, Npc.Value));
            }
        }
