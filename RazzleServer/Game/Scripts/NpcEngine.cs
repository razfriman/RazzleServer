    ...
    public class NpcEngine
    {
        ...
        public bool ExecuteScriptForNpc()
        {
            ...
            try
            {
                if (IsShop)
                {
                    Client.Send(Packets.ShowShop((ShopScript)ScriptInstance, NpcId));
                }
        ...
        private void SendOk(string text) => Client.Send(MapleNpc.GetNpcTalk(NpcId, 0, MapleNpc.ChatType.None, text));

        private void SendNext(string text) => Client.Send(MapleNpc.GetNpcTalk(NpcId, 0, MapleNpc.ChatType.None, text, next: true));

        private void SendPrev(string text) => Client.Send(MapleNpc.GetNpcTalk(NpcId, 0, MapleNpc.ChatType.None, text, prev: true));

        private void SendNextPrev(string text) => Client.Send(MapleNpc.GetNpcTalk(NpcId, 0, MapleNpc.ChatType.None, text, next: true, prev: true));

        private void SendYesNo(string text) => Client.Send(MapleNpc.GetNpcTalk(NpcId, 2, MapleNpc.ChatType.None, text));

        private void SendSimple(string text) => Client.Send(MapleNpc.GetNpcTalk(NpcId, 5, MapleNpc.ChatType.None, text));

        private void SendGetNumber(string text, int defaultValue, int minValue, int maxValue) => Client.Send(MapleNpc.GetNpcTalkNum(NpcId, text, defaultValue, minValue, maxValue));

        private void SendAskText(string text, int min, int max, string textboxText = "") => Client.Send(MapleNpc.GetNpcTalkAskText(NpcId, text, min, max, textboxText));
            ...
            if (shopItems != null)
            {
                if (index >= 0 && index < shopItems.Count)
                {
                    ...
                    if (item.Id == purchaseId)
                    {
                        ...
                        Client.Send(Packets.ShopTransactionResponse(response));
                    }
                }
            }
        }
