...
namespace RazzleServer.Player
{
    public class MapleCharacter : Character
    {
        ...
        public void LoggedIn()
        {
            ...
            Client.Send(SkillMacro.ShowSkillMacros(SkillMacros));
        ...
        public void SendMessage(string message, byte type = 0) => Client.Send(ServerNotice(message, type));
        ...
        public void SendWhiteMessage(string message) => Client.Send(SystemMessage(message, 0x0B));
        ...
        public bool CreateGuild(string guildName)
        {
            ...
            AllianceRank = 5;
            SaveToDatabase(this);
            Client.Send(guild.GenerateGuildDataPacket());
        ...
        public void GainExp(int exp, bool show = false, bool fromMonster = false)
        {
            ...
            if (show)
            {
                if (fromMonster)
                {
                    Client.Send(ShowExpFromMonster(exp));
                }
                else
                {
        ...
        public void IncreaseSkillLevel(int skillId, byte amount = 1, bool updateToClient = true)
        {
            ...
            if (_skills.TryGetValue(skillId, out skill))
            {
                skill.Level += amount;
                if (updateToClient)
                    Client.Send(Skill.UpdateSingleSkill(skill));
                Stats.Recalculate(this);
            }
            else
            {
        ...
        public void SetSkillLevel(int skillId, byte level, byte masterLevel = 0, bool updateToClient = true)
        {
            ...
            if (updateToClient)
                Client.Send(Skill.UpdateSkills(new List<Skill>() { skill }));
            Stats.Recalculate(this);
        }
        ...
        public void AddSkills(List<Skill> addSkills, bool updateToClient = true)
        {
            ...
            if (updateToClient) Client.Send(Skill.UpdateSkills(newSkills));
            Stats.Recalculate(this);
        }
        ...
        public void AddCooldown(int skillId, uint duration, DateTime? nStartTime = null) //duration in MS
        {
            ...
            if (!_cooldowns.ContainsKey(skillId))
            {
                ...
                Client.Send(Skill.ShowCooldown(skillId, duration / 1000));
            }
        }
        ...
        public void RemoveCooldown(int skillId)
        {
            if (_cooldowns.Remove(skillId))
                Client.Send(Skill.ShowCooldown(skillId, 0));
        }
        ...
        public void GiveBuff(Buff buff)
        {
            ...
            switch (buff.SkillId)
            {
                ...
                case Berserker.EVIL_EYE_OF_DOMINATION:
                    Client.Send(Buff.GiveEvilEyeBuff(buff));
                    break;
                case Berserker.CROSS_SURGE:
                    Client.Send(Buff.GiveCrossSurgeBuff(buff, this, buff.Effect));
                    break;
                case DarkKnight.FINAL_PACT2:
                    Client.Send(Buff.GiveFinalPactBuff(buff));
                    break;
                case Priest.HOLY_MAGIC_SHELL:
                    ...
                    Client.Send(Buff.GiveBuff(buff));
                    break;
                default:
                    Client.Send(Buff.GiveBuff(buff));
        ...
        public Buff CancelBuff(int skillId)
        {
            ...
            buff.CancelRemoveBuffSchedule();
            Client.Send(Buff.CancelBuff(buff));
            Stats.Recalculate(this);
        ...
        public bool ForceCompleteQuest(MapleQuest quest, int npcId, int nextQuest = 0)
        {
            ...
            if (_startedQuests.ContainsKey(questId))
            {
                ...
                Client.Send(quest.Update());
                Client.Send(quest.UpdateFinish(npcId, nextQuest));
                return true;
            }
            ...
            lock (_hpLock)
            {
                ...
                if (Party != null)
                {
                    ...
                    if (members.Any())
                    {
                        ...
                        {
                            member.Client.Send(partyHpUpdatePacket);
                        }
                    }
                }
            }
        ...
        public void HandleDeath()
        {
            ...
            if (Job == JobConstants.DARKKNIGHT && !_cooldowns.ContainsKey(DarkKnight.FINAL_PACT2))
            {
                ...
                if (skillLevel > 0)
                {
                    ...
                    GiveBuff(buff);
                    Client.Send(Skill.ShowBuffEffect(DarkKnight.FINAL_PACT2, Level, null, true));
                    ActionState = ActionState.ENABLED;
                    HP = Stats.MaxHp;
                    AddMP(Stats.MaxMp);
                    return;
        ...
        private void FinalPactHook(int oldHp, int newHp)
        {
            ...
            if (skillLevel > 0)
            {
                ...
                if (oldPercent < boundary && newPercent >= boundary) //hp went from under to above the boundary
                    Client.Send(Skill.ShowBuffEffect(DarkKnight.FINAL_PACT, Level, skillLevel, true)); //TODO: show other ppl (dont know packet)
                else if (oldPercent >= boundary && newPercent < boundary) //hp went from above to under the boundary
                    Client.Send(Skill.ShowBuffEffect(DarkKnight.FINAL_PACT, Level, skillLevel, false)); //TODO: show other ppl (dont know packet)               
            }
        }
        ...
        public static void EnterChannel(MapleClient c)
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.ENTER_MAP);
        ...
        public static void SendCSInfo(MapleClient c)
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.ENTER_CASH_SHOP);
        ...
        public static void UpdateStats(MapleClient c, SortedDictionary<MapleCharacterStat, int> stats, bool enableActions)
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.UPDATE_STATS);
        ...
        public static PacketWriter RemovePlayerFromMap(int Id)
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.REMOVE_PLAYER);
            pw.WriteInt(Id);
            return pw;
        }
        ...
        {
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.SERVER_NOTICE); // SERVER_MESSAGE
            pw.WriteShort(type);
        ...
        public static PacketWriter ServerNotice(string message, byte type, int channel = 0, bool whisperIcon = false)
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.SERVER_NOTICE);
        ...
        public static PacketWriter SpawnPlayer(MapleCharacter chr)
        {
            ...
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.SPAWN_PLAYER);
            pw.WriteInt(chr.ID);
            pw.WriteByte(chr.Level);
        ...
        {
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.KEYMAP);
        ...
        {
            var pw = new PacketWriter(); pw.WriteHeader(SMSGHeader.QUICK_SLOT);
            pw.WriteByte(1);
            if (binds.Length == 28)
            {
                for (int i = 0; i < 28; i++)
        ...
        public void IncreaseCriticalGrowth(bool fromAttack)
        {
            ...
            if (criticalGrowth != null)
            {
                if (Stats.CritRate + criticalGrowth.Stacks < 100)
                {
                    ...
                    Client.Send(Buff.GiveBuff(criticalGrowth));
                }
                else if (fromAttack)
                {
